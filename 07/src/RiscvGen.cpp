...
std::vector <std::string> current_func = {"main"};
int end_gen = 0;
    ...
    if(is_gvar){
        ...
        if(is_array && !(is_addr || is_item)){
            ...
            if(is_array && is_addr){
                ...
                //全局变量没有PTR
                std::string op = basety == BTY_CHAR   ? "lb"  : 
        ...
        bool is_ptr = basety == BTY_INT_PTR || basety == BTY_BOOL_PTR || 
                      basety == BTY_FLOAT_PTR || basety == BTY_CHAR_PTR;
        ...
        std::string op = basety == BTY_CHAR  || basety == BTY_CHAR_PTR ? "lb" : 
        ...
        if(is_float || is_double){
            ...
            cact_basety_t basety = BTY_FLOAT;
...
void RiscvGen:: reg_to_var(std::string var_name,std::string reg_name){
    ...
    if(is_gvar){
        basety = semantic_analysis.Gvar_array[order].basety;
        std::string op = basety == BTY_CHAR   ? "sb"  :
        ...
        basety = semantic_analysis.Lvar_array[order].basety;
        std::string op = basety == BTY_CHAR   || basety == BTY_CHAR_PTR   ? "sb"  :
                         basety == BTY_FLOAT  || basety == BTY_FLOAT_PTR  ? "fsw" :
                         basety == BTY_INT    || basety == BTY_INT_PTR    ? "sw"  :
                         basety == BTY_BOOL   || basety == BTY_BOOL_PTR   ? "sb"  : "" ;
        std::string stack_offset = "-"+std::to_string(semantic_analysis.Lvar_array[order].offset);
        bool is_ptr = basety == BTY_CHAR_PTR || basety == BTY_FLOAT_PTR || basety == BTY_INT_PTR || basety == BTY_BOOL_PTR ;
        if(is_item){
            if(is_ptr){
...
void RiscvGen:: Gen_FuncBegin(IR_code_t &irc){
    ...
    //for03
    if(func_name!="main") current_func.push_back(func_name);
    //
...
void RiscvGen:: Gen_FuncEnd(IR_code_t &irc){
    ...
    // for03
    if(current_func.size()>1){current_func.pop_back();}
}
...
void RiscvGen:: Gen_Call(IR_code_t &irc){
    ...
    if(IR_temp.length()!=0){
        std::string reg = (basety == BTY_FLOAT) ? "fa0" : "a0";
        std::string op = (basety == BTY_CHAR) ? "sb" :
                         (basety == BTY_FLOAT) ? "fsw" :
                         (basety == BTY_BOOL) ? "sb" : 
                         "sw";
...
void RiscvGen:: Gen_Return(IR_code_t &irc){
    ...
    cact_basety_t basety = irc.basety;
    std::string reg_name = (basety == BTY_FLOAT) ? "fa0" : "a0";
    std::string var_name = irc.result;
    std::string func_name = current_func.back();
    var_to_reg(var_name,reg_name);
    // for32
    ASM_array.push_back("ld      ra, " + std::to_string(frame_size-8) + "(sp)");
    ASM_array.push_back("ld      s0, " + std::to_string(frame_size-16) + "(sp)");
    ...
    ASM_array.push_back(".size   " + func_name + ", .-" + func_name);
    // for03
    if(current_func.back() == "main"){
        ASM_array.push_back("ld      ra, " + std::to_string(frame_size-8) + "(sp)");
        ASM_array.push_back("ld      s0, " + std::to_string(frame_size-16) + "(sp)");
        ASM_array.push_back("addi    sp, sp, " + std::to_string(frame_size));
        ASM_array.push_back("jr      ra");
        ASM_array.push_back(".size   main, .-main");
        end_gen = 1;
    }
}
...
void RiscvGen:: Gen_Operation(IR_code_t &irc){
    ...
    //neg系列和not都是伪代码
    switch(irc.IRop){
        case IR_ADD: op = is_float ? "fadd.s" : "addw"; break;
        case IR_SUB: op = is_float ? "fsub.s" : "subw"; break;
        case IR_MUL: op = is_float ? "fmul.s" : "mul" ; break;
        case IR_DIV: op = is_float ? "fdiv.s" : "div" ; break;
        case IR_MOD: op = "rem"; break;
        case IR_AND: op = "and"; break;
        case IR_OR:  op = "or" ; break;
        // case IR_NOT: op = "not"; break;
        case IR_NEG: op = is_float ? "fneg.s" : "neg"; break;
        default: break;
    }
    ...
    if(is_unary){
        if(irc.IRop==IR_NOT){
            ...
            ASM_array.push_back("seqz    "+reg0+", "+reg1+""); // 原为xori
        }
        else{
...
void RiscvGen:: Gen_Branch(IR_code_t &irc){
    ...
    switch(irc.IRop){
        case IR_BEQ: op = is_float ? "feq.s" : "beq" ; break;
        case IR_BNE: op = is_float ? "feq.s" : "bne" ; break;
        case IR_BLT: op = is_float ? "flt.s" : "blt" ; break;
        case IR_BLE: op = is_float ? "fle.s" : "ble" ; break;
        case IR_BGT: op = is_float ? "flt.s" : "blt" ; reverse = true; break;
        case IR_BGE: op = is_float ? "fle.s" : "ble" ; reverse = true; break;
        default: break;
    }
    if(is_float){
        //需要额外的临时寄存器t5存储比较结果
...
void RiscvGen:: Gen_G_Alloc(IR_code_t &irc){
    ...
    //分区信息
    std::string sec_str =   is_const ? ".section .rodata" :
                            is_inited ? ".data" :
                            ".bss";
    //汇编语言中，数组都按照8字节对齐。对bool类型单个变量，不用给出align
    std::string align_str = (len != 1) ? ".align  3" : 
                            (basety != BTY_BOOL || basety != BTY_CHAR)    ? ".align  2" :
                            "";
...
void RiscvGen:: Gen_L_Alloc(IR_code_t &irc){
    ...
    std::string initval_str = irc.arg1;
    bool is_inited = initval_str.size()!=0;
    bool is_lvar = IR_name[0] == LOCAL_PREFIX;
    ...
    if(len > 1){
        ...
        //前缀，是否使用浮点寄存器
        std::string prefix = (basety == BTY_FLOAT) ? "f" : ""; 
...
void RiscvGen:: Cnt_Temp_Lvar(std::string var_name){
    ...
    if(is_temp){
        ...
        basety = semantic_analysis.Lvar_array[order].basety;
        len = semantic_analysis.Lvar_array[order].length;
    }
    //这里设计bool使用1byte
    size_t size = (typeutils.basety_to_size)[basety];
    //对齐到size大小
